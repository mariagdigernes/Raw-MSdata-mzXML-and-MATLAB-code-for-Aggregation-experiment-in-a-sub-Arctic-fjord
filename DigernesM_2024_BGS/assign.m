...
charge=1.0072765*ESI_mode; %mass of H ?
...
%elements: C,H,O,N,S,P,Se,Na,Cl,C13
%el matrix is number of elements for each possible compound
el_matrix=[e_array{1}(:) e_array{2}(:) e_array{3}(:) e_array{4}(:) e_array{5}(:) e_array{6}(:) e_array{7}(:) e_array{8}(:) e_array{9}(:) e_array{10}(:)]; % el matrix is theoretical elements. puts 10 elements (C,H,O,N,S,P,Se,Na,Cl,C13) and every possible combination on each row by increasing 1 carbon consecutively every row (first column) but leaving all other columns alone. then increasing 1 Hydrogen consecutively on second column and so forth. (regardless of samples)
%%
formula_features=zeros(size(el_matrix,1),1); %initialize size of formula features to match el matrix
valence_tot=zeros(size(el_matrix,1),1); %initializes valence size to match el matrix
formula_features=formula_features+charge; %compensates mass change during ionisation. 
for els=1:no_els % 1 to number of elements (10)
    formula_features(:,1) = formula_features(:,1)+el_matrix(:,els)*elements(els,3);% calculates exact mass by multiplying number of element *mass. for each 2million compound possibilities on el matrix (each row on el matrix is a possible compound).
    valence_tot=valence_tot+el_matrix(:,els)*elements(els,4);% look for elements valence (on csv assign)
end
valence_mod=mod(valence_tot,2); %valence number has to be even. zero if even, 1 if odd. modulous remainder
%% 
%calculates possibilities of elemental composition matrix
formula_features(:,2) = el_matrix(:,2)./(el_matrix(:,1)+el_matrix(:,10)); % column 2 is H/C12+c13. note el matrix column 2 is 
formula_features(:,3) = el_matrix(:,3)./(el_matrix(:,1)+el_matrix(:,10)); %O/C12+C13
formula_features(:,4) = formula_features(:,1)*14/14.01565;    % Kendrick mass. reduction of mass. mass of possible compounds. 
...
formula_features(:,7) = 1+0.5*(2*el_matrix(:,1)+2*el_matrix(:,10)-el_matrix(:,2)-el_matrix(:,4)-el_matrix(:,6));  %DBE
formula_features(:,8) = formula_features(:,7)-el_matrix(:,3);   %DBE-O
formula_features(:,9) = el_matrix(:,1)*C13prob/C12prob;  %predicted 13C pattern

DBE_AIm=1+el_matrix(:,1)+el_matrix(:,10)-0.5*el_matrix(:,3)-el_matrix(:,5)-el_matrix(:,7)-0.5*(el_matrix(:,4)+el_matrix(:,2)+el_matrix(:,6));
C_AIm=el_matrix(:,1)+el_matrix(:,10)-0.5*el_matrix(:,3)-el_matrix(:,5)-el_matrix(:,4)-el_matrix(:,7)-el_matrix(:,6);

formula_features(:,10) = DBE_AIm./C_AIm;  %mod AI
formula_features(formula_features(:,10)<0,10)=0;  %removes negative AI


formula=formula_features; %formula features are ten columns with each column representing a characteristic of elemental compositions (H/C, DBE,13C,KMD etc) 
elements_used=el_matrix;

% filters to remove bad formulas. NB cx are logical operators so if true operations!
c1=formula(:,1)<min_mass | formula(:,1)>max_mass; %criteria 1 mass filter. min mass??
c2=formula(:,2)<0.3 | formula(:,2)>2.2; %H/C
c3=formula(:,3)>1; %O/C
c4=valence_mod==1; %1 if odd # of valence electrons
c5=formula_features(:,8)>10|formula_features(:,8)<(-10); %DBE-O filter away if >10 or <-10
...
conditions=c1+c2+c3+c4+c5+c6+c7+c8>0; %sums conditions and create big mama filter

formula(conditions,:)=[]; %if condition is true (1) replace with empty (remove)
...
for n=1:nosamps 
    ...
    for rep=1:noreps
        ...
        samplemax=max(datsmpraw(:,2));
        
        masses=datsmpraw(:,1);
        intensities=datsmpraw(:,2);
        mass_defects=datsmpraw(:,1)-floor(datsmpraw(:,1));
        c13d2=(13.00335-12)/2;
        ppm_tol=2;
        ...
        for m=1:length(masses)
            if mass_defects(m)>0.4&mass_defects(m)<0.8
                ...
                mono_pos=find(m_diff==min(m_diff));
        ...
        if length(noise_peaks)>100
            noise_quant=1*quantile(noise_peaks(:,2),0.99);
        else
            noise_quant=0;
        end
...
%%
refilter=sum_all>0 & elements_used(:,10)==0; %filters peaks to be in at least x samples (changed from sum_all>0 to 1), no isotopologues
sum(refilter)
...
for smp=1:nosamps
    ...
    for rep=1:noreps
        ...
        norm_rep(:,rep)=clean_rep/sum(clean_rep)*1e6;
    end
